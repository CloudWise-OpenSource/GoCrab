...
func genRouterCode() {
	...
	GoCrab.GlobalControllerRouter["` + k + `"] = append(GoCrab.GlobalControllerRouter["` + k + `"],
		GoCrab.ControllerComments{
...
func compareFile(pkgRealpath string) bool {
	if !Helpers.FileExists(path.Join(workPath, "routers", commentFilename)) {
		return true
	}
	if Helpers.FileExists(path.Join(workPath, lastupdateFilename)) {
