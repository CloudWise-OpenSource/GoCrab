...
var (
	...
	// these GoCrab.Controller's methods shouldn't reflect to AutoRouter
	exceptMethod = []string{"Init", "Prepare", "Finish", "Render", "RenderString",
		"RenderBytes", "Redirect", "Abort", "StopRun", "UrlFor", "ServeJson", "ServeJsonp",
		"ServeXml", "Input", "ParseForm", "GetString", "GetStrings", "GetInt", "GetBool",
		"GetFloat", "GetFile", "SaveToFile", "IsAjax", "GetSecureCookie",
		"SetSecureCookie", "GetControllerAndAction"}
...
func (p *ControllerRegistor) Add(pattern string, c ControllerInterface, mappingMethods ...string) {
	...
	route.routerType = routerTypeGoCrab
...
// example GoCrab.AddAuto(&MainContorlller{}),
...
// example GoCrab.AddAutoPrefix("/admin",&MainContorlller{}),
// MainController has method List and Page.
...
func (p *ControllerRegistor) AddAutoPrefix(prefix string, c ControllerInterface) {
	...
	for i := 0; i < rt.NumMethod(); i++ {
		if !Helpers.InSlice(rt.Method(i).Name, exceptMethod) {
			route := &controllerInfo{}
			route.routerType = routerTypeGoCrab
...
func (p *ControllerRegistor) geturl(t *Tree, url, controllName, methodName string, params map[string]string, httpMethod string) (bool, string) {
	...
	for _, l := range t.leaves {
		if c, ok := l.runObject.(*controllerInfo); ok {
			if c.routerType == routerTypeGoCrab &&
...
func (p *ControllerRegistor) ServeHTTP(rw http.ResponseWriter, r *http.Request) {
	...
	if RunMode == RUNMODE_DEV {
		w.Header().Set("Server", GoCrabServerName)
	}
...
Admin:
	...
	if RunMode == RUNMODE_DEV || AccessLogs {
		var devinfo string
		if findrouter {
			if routerInfo != nil {
...
func (p *ControllerRegistor) recoverPanic(context *context.Context) {
